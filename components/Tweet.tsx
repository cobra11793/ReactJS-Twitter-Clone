...
function Tweet({ tweet }: Props) {
    ...
    return (
        <div className='flex flex-col space-x-3 border-y p-5 overflow-y-auto border-gray-100'>
            ...
            {comments?.length > 0 && (
                <div className='my-2 mt-5 max-h-44 space-y-5 border-t overflow-y-scroll border-gray-100 p-5'>
                    {comments.map(comment => (
                        <div key={comment._id} className='relative flex space-x-2'>
                            <hr className='absolute left-5 top-10 h-8 border-x border-twitter/20' />
                            <img src={comment.profileImg} className='mt-2 h-7 w-7 object-cover rounded-full' />
                            <div>
                                <div className='flex items-center space-x-1'>
